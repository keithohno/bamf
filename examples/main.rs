...
fn main() {
    let input = Vector::from_vec(vec![0.0, 1.0, 2.0]);
    ...
    let bad_target = Vector::from_vec(vec![0.5, 0.5]);
    let good_target = Vector::from_vec(vec![0.01, 0.99]);
    ...
    let vector = Vector::from_vec(vec![1.0, 2.0]);
...
fn repeat_nn_single_input() {
    ...
    let input = Vector::from_vec(vec![0.0, 1.0, 2.0]);
    let mut nn1 = NeuralNetwork::new(&mut weights, &input, Vector::from_vec(vec![0.0, 1.0]));
    for _ in 0..100 {
        nn1.train();
    }
